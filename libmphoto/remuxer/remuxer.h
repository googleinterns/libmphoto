...
class Remuxer {
 public:
  ...
  // Produces a motion photo using supplied encoded media streams.
  absl::Status GenerateNewMotionPhoto(const absl::string_view still_image_bytes,
                                      const absl::string_view video_image_bytes,
  ...
  absl::Status ReplaceStill(const absl::string_view new_still_image_bytes,
                            const absl::string_view current_motion_photo,
                            std::string *motion_photo);

  // Produces a motion photo from an existing one with the video portion
  // replaced with a new encoded stream.
  absl::Status ReplaceVideo(const absl::string_view new_video_bytes,
                            const absl::string_view current_motion_photo,
                            std::string *motion_photo);
};
